---
uid: 7592
source: "[[Datadog Blog]]"
author: "[[Unknown]]"
date: 2020-06-08
tags: []
category: Platform & SRE
url: https://www.datadoghq.com/blog/upload-download-files-browser-tests/
status: "\U0001F331 Seedling"
---

# Test file uploads and downloads with Datadog Synthetic browser tests

## AI Summary
_No summary available yet._

## Topics
_No topics tagged yet._

## Source
- Blog: [[Datadog Blog]]
- Author: [[Unknown]]
- Original: [Read Full Post](https://www.datadoghq.com/blog/upload-download-files-browser-tests/)
